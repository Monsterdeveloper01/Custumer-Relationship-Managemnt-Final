...
$stmt = $pdo->prepare("SELECT company_email, company_name, name_person
                       FROM crm WHERE marketing_id = :mid");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_email'])) {
    ...
    if (!$data) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE crm SET status='emailed', updated_at=NOW()
    ...
    <form method="post" class="space-y-5">
      <div>
        ...
        <select name="company_email"
          ...
          class="w-full p-3 rounded border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 bg-white">
          <option value="">-- pilih perusahaan --</option>
          <?php foreach ($companies as $c): ?>
            <option value="<?= h($c['company_email']) ?>"
              <?= $selectedCompany && $selectedCompany['company_email']==$c['company_email'] ? 'selected' : '' ?>>
      ...
      <?php endif; ?>
<a href="../View/Login/dashboard.php"
